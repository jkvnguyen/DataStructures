...
public class BinarySearchTree extends LinkedBinaryTree implements Dictionary {
   ...
   * 
   * Find if the key is in the tree, then return the node of that key
   * 
   *  */
  ...
  public Entry insert(Object k, Object x) throws InvalidKeyException {
    ...
    Position insPos = treeSearch(k, root()); //this one only checks the root, so you always start from the root down (to search)
