...
public class HeapPriorityQueue<K,V> extends AbstractPriorityQueue<K,V> {
  ...
  protected void upheapRec( int j ) {
	...
	// Use recursion.  Do NOT use any kind of loop.
	 ...
	 if (j <= 1){
		 return;
	 } else if (compare(heap.get(j), heap.get(p)) >= 1) {
		 return;
	 } else {
		 swap(j, p);
		 upheapRec(p);
  ...
  protected void downheapRec( int j ) {
	...
	// COMPLETE THIS METHOD
	  ...
	  if(!hasLeft(j)){
		  return;
	  } else if(compare(heap.get(smallChildIndex), heap.get(j)) >= 0) {
		  return;
	  } else {
		  swap(j, smallChildIndex);
  ...
  public int updateKey( K currentKey, K newKey ) {
	...
	// You may use loops in this method.
	  ...
	  for( int i = 1; i < heap.size(); i++ ){
		  index = heap.get(i).getKey();
		  if(index == currentKey){
			  Entry<K,V> temp = new PQEntry<>(newKey, heap.get(i).getValue());
			  heap.set(i, temp);
		  }  
	  }
